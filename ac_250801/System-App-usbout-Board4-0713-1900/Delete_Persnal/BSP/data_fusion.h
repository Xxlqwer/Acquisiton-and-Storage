...
/* 宏定义 */
#define STARTUP_TIMEOUT_MS  (3 * 60 * 1000)  //刚启动的授时期限  分钟*60*1000 ms  常量宏定义，编译期替换，不会占用内存
#define STORAGE_CHECK_INTERVAL_MS   (30 * 60 * 1000U)  // 分钟*60*1000，单位ms，用于测试

extern const char* g_lastError;
#define SET_ERROR(msg)  g_lastError = msg;
