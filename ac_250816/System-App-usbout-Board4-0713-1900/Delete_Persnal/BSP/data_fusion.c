...
uint32_t syncIntervalMs = 60*60*1000;  //  分钟x60x1000 ms
...
void MainLoop_StateMachine(void)
{
    ...
    else
    {
				...
				if (systemState != SYSTEM_NORMAL_OPERATION)
        {
				...
				f_mount(NULL, "", 1);
					extern FATFS fs;
					memset(&fs, 0, sizeof(FATFS)); //清空句柄
					printf("f_mount");
					
			// step2: 重置 SDIO 控制器（HAL 建议顺序）
				__HAL_RCC_SDIO_CLK_DISABLE();
				__HAL_RCC_SDIO_FORCE_RESET();
				HAL_Delay(20);
				__HAL_RCC_SDIO_RELEASE_RESET();
				 HAL_Delay(20);
				__HAL_RCC_SDIO_CLK_ENABLE();  // 非常关键！	
				HAL_Delay(20);  // 稍作延时 

			 // step3: 重新初始化 SDIO 外设 + 卡配置
				MX_SDIO_MMC_Init();
        ...
        case STATE_IDLE:
       {
            if (startupTick == 0) 
            {
                startupTick = currentTick;
							printf("[IDLE] 系统启动，准备进入 GNSS 授时流程...\r\n");
							// 启动接收 + 发送命令
							HAL_UART_Receive_IT(&huart6, &gnssByte, 1);
							UM980_StatusCheck();

							 needTimeSync = true;    //关键：允许首次授时！
							appState = STATE_WAIT_GNSS;
							
							break;  
            };

            break;// 若 startupTick 已设置，保持空转
        }
//--------------------------------------------------
				case STATE_WAIT_GNSS:
					{
							if (gnssReady)
							{
									printf("[WAIT_GNSS] GNSS 报文已接收，进入处理阶段...\r\n");
									appState = STATE_GNSS_RECEIVED;
								 // 清除标志，防止二次触发
								gnssReady = false;
								gnssIndex = 0;
							}
							else if (HAL_GetTick() - startupTick >= STARTUP_TIMEOUT_MS) //启动时的授时
							{
									printf("[WAIT_GNSS] 超时未收到 GNSS，fallback 使用 RTC 进入采集模式\r\n");
									...
									currentFileDay = sDate.Date;
										...
										gnssReady = false;
										gnssIndex = 0;
										needTimeSync = false;  //初次启动未授时成功，在到达下一次授时时间前，即使接收到有效GNSS，也禁止再次尝试授时
									appState = STATE_ACQUIRE;
							}
							break;
					}
        ...
        case STATE_GNSS_RECEIVED:
			 {
				  ...
				  // USB 连接时跳过授时动作
					if (hUsbDeviceHS.dev_state == USBD_STATE_CONFIGURED)
					{
							printf("[GNSS] 当前为 USB MSC 模式，忽略 GNSS 报文。\r\n");
					}
					else if (needTimeSync)
				{
					extract_time(gnssBuffer);
					//HAL_GPIO_WritePin(LED0_GPIO_Port, LED0_Pin, GPIO_PIN_SET); // 授时完成指示
					lastSyncTime = HAL_GetTick();  // 更新时间戳
					printf("授时成功,间隔%ums\r\n",syncIntervalMs);
					needTimeSync = false;
				}
				// 清理缓存
        ...
        case STATE_TIMESYNC:
           {
            if (needTimeSync && systemState == SYSTEM_NORMAL_OPERATION) 
            {
               ...
               /* 后续真正写 RTC 的动作在 STATE_GNSS_RECEIVED → extract_time() 中完成，
                 这里不再重复写 RTC，避免双写 */
									lastSyncTime = HAL_GetTick();  //授时成功，立即更新时间戳
									printf("[SYNC] GNSS已接收，等待 GNSS_RECEIVED 处理中更新 RTC\r\n");
									 appState = STATE_GNSS_RECEIVED;  // 解析授时数据的状态
                } 
                else 
                {
                    printf("授时失败，将在10分钟后重试\r\n");        
										lastSyncTime = HAL_GetTick() - (syncIntervalMs - 10 * 60 * 1000); // 使下次授时时间提前：现在 + 10分钟
										 needTimeSync = false;  // 授时失败才清除
										appState = STATE_ACQUIRE;
								}
        ...
        case STATE_ACQUIRE:
            ...
            if (currentTick - lastReportTick >= 60000) 
            {
                ...
                bufferOverrunCount = 0;
							HAL_GPIO_TogglePin(LED0_GPIO_Port, LED0_Pin); // 指示灯
            }
						 /* -------- 2. 定时 GNSS 授时 -------- */
            ...
            if (currentTick  - lastSpaceCheck >  STORAGE_CHECK_INTERVAL_MS) 
            {
              CheckStorageSpace();
            ...
            if (!fileCreated) 
            {
							...
							FIL tmpFile;
							if (FATFS_CreateRawDataFileWithTimestamp(&tmpFile)) 
							{
									rawDataFile = tmpFile;     // 拷贝句柄
									fileCreated = true;
							}
							else
							{
...
void CheckAndCreateNewFileIfNeeded(void)
{
    ...
    if ((currentFileHour != sTime.Hours) || (currentFileDay != sDate.Date))
    {
			...
			fileCreated = false;  // 不论是否成功新建，先重置标志
			
			/* ② 尝试创建新文件 */
			FIL tmpFile;
			if (FATFS_CreateRawDataFileWithTimestamp(&tmpFile))   // 传地址
			{
				 rawDataFile = tmpFile;     // 结构体拷贝
					fileCreated = true;
				 // 更新当前时间
				currentFileHour = sTime.Hours;
				currentFileDay  = sDate.Date;
...
void FlushBufferToSD(uint8_t bufferIndex)
{
    ...
    {
        res = f_write(&rawDataFile, (const void*)buffer->data, bytesToWrite, &bytesWritten);
        if (res == FR_OK && bytesWritten == bytesToWrite)
            break;

        printf("[WARN] 写入失败，重试 %d 次... 错误码: %d\n", attempt + 1, res);
        HAL_Delay(2);
    }
    ...
    if (res != FR_OK || bytesWritten != bytesToWrite) 
    {
        printf("[ERROR] 写入失败: 预期 %u 字节, 实际 %u 字节, 错误码 %d\n", 
               bytesToWrite, bytesWritten, res);
			
			
			// 如果是严重错误码，触发软复位
			if (res == FR_DENIED || res == FR_INT_ERR || res == FR_INVALID_OBJECT) 
			{
					printf("[FATAL] 写入失败无法恢复，系统软复位中...\n");
					Soft_Reset();  // 复位函数
			}
    }
    ...
    if (++writeCount >= 10) 
    {
        FRESULT syncRes = f_sync(&rawDataFile);
        if (syncRes != FR_OK) 
        {
            printf("[WARN] 文件同步失败，错误码: %d\n", syncRes);
        }
        writeCount = 0;
    }
}
...
void Reset_USB3320(void)
{
    // 拉低复位脚
    HAL_GPIO_WritePin(GPIOC, GPIO_PIN_1, GPIO_PIN_RESET);
    HAL_Delay(10);  // 保持低电平 >=10ms

    // 拉高释放复位
    HAL_GPIO_WritePin(GPIOC, GPIO_PIN_1, GPIO_PIN_SET);
    HAL_Delay(20);  // 等待 USB3320 初始化完成
}