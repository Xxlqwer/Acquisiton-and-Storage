...
volatile bool gnssStarted = false;  // 是否已成功接收到有效 GNSS 报文
volatile bool gnssReady = false;    // 当前报文是否有效
...
bool PerformTimeSync(void) 
{
		...
		HAL_UART_Receive_IT(&huart6, &gnssByte, 1);  // <- 必须加这个
	 
//		if (!gnssStarted)
//			{
//					UM980_StatusCheck(); // 发送指令
//			}
    UM980_StatusCheck(); // 发送指令
    // 2. 等待并解析时间信息
    uint32_t start = HAL_GetTick();
		printf("等待GNSS消息...\r\n");
    while ((HAL_GetTick() - start <2000)) //2秒
		{
			if (gnssReady)
        {
            break;
...
//            }
            ...
            if (gnssByte == '\n')  // 结束符
            {
               ...
               // if (needTimeSync && valid_RxCplt((const char*)gnssBuffer)) {
							if (valid_RxCplt((const char*)gnssBuffer)) {	
											//printf("[GNSS VALID] ：%s", gnssBuffer);  // 推荐打印一下，便于调试
											gnssReady = true;  // STATE_GNSS_RECEIVED区分 USB 状态，保证 PerformTimeSync 可用
										if (!gnssStarted) {
												gnssStarted = true;
												printf("[GNSS INIT] 首次接收到有效 GNSS 报文，标记 gnssStarted = true\r\n");
										}
								}
        ...
        HAL_UART_Receive_IT(&huart6, &gnssByte, 1); // 继续接收下一个字节
	}
}
