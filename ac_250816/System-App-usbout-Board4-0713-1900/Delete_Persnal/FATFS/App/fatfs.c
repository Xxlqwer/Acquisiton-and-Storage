...
bool FATFS_CreateRawDataFileWithTimestamp(FIL* outFile)
{
    FRESULT res;
    char filename[64];
    char folder[32];
    ...
    // 打开文件（追加模式）
    res = f_open(outFile, filename, FA_OPEN_ALWAYS | FA_WRITE);
    if (res != FR_OK) {
        printf("[ERROR] 无法打开文件: %s，错误码: %d\r\n", filename, res);
        return false;
    }

    // 移动到文件末尾
    res = f_lseek(outFile, f_size(outFile));
    if (res != FR_OK) {
        printf("[ERROR] 定位文件末尾失败: %d\r\n", res);
        f_close(outFile);
        return false;
    }
    ...
    // 成功提示
    if (f_size(outFile) == 0) {
        printf("[INFO] 新建HEX数据文件: %s\r\n", filename);
    } else {
        printf("[INFO] 续写HEX数据文件: %s, 当前偏移: %lu Bytes\r\n", filename, f_tell(outFile));
    }
...
FRESULT DeleteOldestFile(void) 
{
    ...
    DIR dir;
    FILINFO fno;
    FILINFO oldest_fno = {0};  // 记录最旧文件
    char boardPath[64];
    char  oldestPath[128] = {0};
		FRESULT res;
		
		  /* 1. 拼目录 0:/Board<id> */
    snprintf(boardPath, sizeof(boardPath), "0:/Board%u", g_config.board_id);
    
		 /* 2. 打开目录 */
     res = f_opendir(&dir, boardPath);
			if (res != FR_OK) {
        printf("打开目录失败: %s (err=%d)\r\n", boardPath, res);
        return res;
    }
			
//#if _USE_LFN
//    char lfnBuf[128];               // 长度必须够长
//    fno.lfname = lfnBuf;
...
//#endif		
   ...
   // 3. 遍历查找最旧文件
    while (1) {
        ...
        if (res != FR_OK || fno.fname[0] == 0) break;  // 出错或结束
        if (fno.fname[0] == '.') continue;             // 跳过 . 和 ..

        // 筛选 hex 或包含 DATA_ 的文件
        if (!(strstr(fno.fname, ".hex") || strstr(fno.fname, "DATA_")))
            continue;

        // 判断是否更旧
        bool isOlder =
            oldest_fno.fname[0] == 0 ||
            (fno.fdate < oldest_fno.fdate) ||
            (fno.fdate == oldest_fno.fdate && fno.ftime < oldest_fno.ftime) ||
            (fno.fdate == oldest_fno.fdate && fno.ftime == oldest_fno.ftime &&
             strcmp(fno.fname, oldest_fno.fname) < 0);

        if (isOlder) {
            oldest_fno = fno;
            snprintf(oldestPath, sizeof(oldestPath), "%s/%s", boardPath, fno.fname);
        }
    }
    f_closedir(&dir);
...
//    }
   ...
   // 检查是否低于阈值
		//g_config.space_threshold_mb = 29696; //29 * 1024 = 29696（单位：MB）
